...
def get_tp_fp_fn_tn(net_output, gt, axes=None, mask=None, square=False):
    ...
    with torch.no_grad():
        ...
        if all([i == j for i, j in zip(net_output.shape, gt.shape)]):
            ...
            y_onehot.scatter_(1, gt, 1)
            '''
            save_path = '/home/sunxuezheng/sda/documents/nnUNetFrame1/feature_map/task207_temp1'
            label_map1 = y_onehot[0,0,:,:,:].cpu().detach().numpy().astype(np.float32)
            label_map2 = y_onehot[0,1,:,:,:].cpu().detach().numpy().astype(np.float32)
            label_map3 = y_onehot[0,2,:,:,:].cpu().detach().numpy().astype(np.float32)
            label_map4 = y_onehot[0,3,:,:,:].cpu().detach().numpy().astype(np.float32)
            f_path1 = save_path+'/'+ 'label11'+'.nii.gz'
            f_path2 = save_path+'/'+ 'label22'+'.nii.gz'
            f_path3 = save_path+'/'+ 'label33'+'.nii.gz'
            f_path4 = save_path+'/'+ 'label44'+'.nii.gz'
            save_arry_as_nii(label_map1,f_path1)
            save_arry_as_nii(label_map2,f_path2)
            save_arry_as_nii(label_map3,f_path3)
            save_arry_as_nii(label_map4,f_path4)
            '''
